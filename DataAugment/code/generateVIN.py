...
position_new = [random.gauss(315,5),random.gauss(230,2)]
position_old = [random.gauss(285,5),random.gauss(315,2)]
...
def drawText(vin, filename,i,version):     #每幅图
    #try:
        ...
        region.save(filename + '/vin/' + str(i) + '_' + version + '.jpg')
        file_txt = open(filename + '/vin/' + str(i) + '_' + version +'.txt','w')
        file_txt.write(vin)
        file_txt.close()
...
if __name__ == '__main__':
    filename = '/data/linkface/OcrData/VINData'
    #清空生成文件夹
    shutil.rmtree(filename + '/vin/')  
    os.mkdir(filename + '/vin/')
    shutil.rmtree(filename + '/wholeImage/')  
    os.mkdir(filename + '/wholeImage/')
    file = open("./assets/vintxt.txt")       #输入信息
    i = 0
