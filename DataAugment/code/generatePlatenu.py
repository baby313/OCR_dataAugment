...
def drawText(platenu, filename,i,version):     #每幅图
    #try:
        ...
        region.save(filename + '/platenu/' + str(i) + '_' + version + '.jpg')
        file_txt = open(filename + '/platenu/' + str(i) + '_' + version +'.txt','w')
        file_txt.write(platenu)
        file_txt.close()
...
def generateImage(platenus, output_folder, start_index):
    i = start_index
    for platenu in platenus:
        i += 1
        ...
        if a < 0.7:
            ...
            version = 'old'
        drawText(platenu, output_folder,i,version)

        if i % 100 == 0:
            print("Thread: {0} generate {1}".format(start_index, i))

if __name__ == '__main__':
    filename = '/data/linkface/OcrData/PlatenuData'
    #清空生成文件夹
    shutil.rmtree(filename + '/platenu/')  
    os.mkdir(filename + '/platenu/')
    #shutil.rmtree(filename + '/wholeImage/')  
    #os.mkdir(filename + '/wholeImage/')
    file = open("./assets/platenu_txt.txt")       #输入信息
    i = 0
    ...
    try:
        ...
        for platenu in file:
            i += 1             #生成第i个文件
            platenus.append(platenu)

            if i % 50000 == 0:
                thread.start_new_thread(generateImage, (platenus, filename, i))
                platenus = []
            #生成文件存放位置
