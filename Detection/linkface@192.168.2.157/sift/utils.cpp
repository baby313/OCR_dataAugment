...
char* prepend_path( const char* path, const char* file )
{
  ...
  char pp[100];
  char* pathname =  pp;//calloc( n, sizeof(char) );
...
*/
char* basename_ut(const char* pathname )
{
...
void display_big_img( IplImage* img, char* title )
{
  ...
  scr_width = img->width/0.8;//gdk_screen_get_width( scr );
  scr_height = img->height/0.8;//gdk_screen_get_height( scr );
